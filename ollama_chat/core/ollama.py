...
def ask_ollama(
    ...
    *,
    ...
    return ask_ollama_with_conversation(
        conversation,
        selected_model,
        temperature,
        prompt_template,
...
def ask_ollama_with_conversation(
    ...
    *,
    ...
    if ctx.no_system_role and len(conversation) > 1 and conversation[0]["role"] == "system" and not conversation[0]["content"] is None and not conversation[1]["content"] is None:
    ...
    if (ctx.use_openai or ctx.use_azure_openai) and not model_is_an_ollama_model:
        if ctx.verbose:
            utils.on_print("Using OpenAI API for conversation generation.", Fore.WHITE + Style.DIM)
    ...
    if not ctx.syntax_highlighting:
        if ctx.interactive_mode and not no_bot_prompt:
            ...
            if prompt_color:
                utils.on_stdout_write("", prompt_color)
            else:
                utils.on_stdout_write("", Style.RESET_ALL)
    ...
    if (ctx.use_openai or ctx.use_azure_openai) and not model_is_an_ollama_model:
        ...
        while not completion_done:
            ...
            if bot_response and bot_response_is_tool_calls:
                ...
                if ctx.verbose:
                    utils.on_print(f"Bot response: {bot_response}", Fore.WHITE + Style.DIM)
    ...
    if ctx.verbose and think:
        utils.on_print("Thinking...", Fore.WHITE + Style.DIM)
    ...
    try:
        ...
        if "does not support tools" in str(e):
            ...
            utils.on_print(f"An error occurred during the conversation: {e}", Fore.RED)
            return ""
        ...
        try:
            if stream_active and len(tools) == 0:
                if ctx.alternate_model:
                    utils.on_print(f"Response from model: {model}\n")
                chunk_count = 0
                for chunk in stream:
                ...
                if len(tool_calls) > 0:
                    ...
                    if ctx.verbose:
                        utils.on_print(f"Tool calls: {tool_calls}", Fore.WHITE + Style.DIM)
...
def handle_tool_response(
    ...
    *,
    ...
    for tool_call in bot_response:
        ...
        for tool in tools:
            if 'type' in tool and tool['type'] == 'function' and 'function' in tool and 'name' in tool['function'] and tool['function']['name'] == tool_name:
                ...
                if ctx.verbose:
                    utils.on_print(f"[DEBUG] Initial parameters: {parameters}", Fore.CYAN + Style.DIM)
                    utils.on_print(f"[DEBUG] Parameters type: {type(parameters)}", Fore.CYAN + Style.DIM)

                # if parameters is a string, convert it to a dictionary
                if isinstance(parameters, str):
                    if ctx.verbose:
                        utils.on_print("[DEBUG] Converting string parameters to dict", Fore.CYAN + Style.DIM)
                    try:
                        parameters = utils.extract_json(parameters,  ctx=ctx)
                        if ctx.verbose:
                            utils.on_print(f"[DEBUG] After extract_json: {parameters} (type: {type(parameters)})", Fore.CYAN + Style.DIM)
                    except Exception as e:
                        if ctx.verbose:
                            utils.on_print(f"[DEBUG] extract_json failed: {e}, using empty dict", Fore.CYAN + Style.DIM)
                        parameters = {}
                ...
                if isinstance(parameters, list):
                    if ctx.verbose:
                        utils.on_print("[DEBUG] Parameters is a list, attempting to convert to dict", Fore.CYAN + Style.DIM)
                    ...
                    if 'parameters' in tool.get('function', {}) and 'properties' in tool['function']['parameters']:
                        ...
                        if ctx.verbose:
                            utils.on_print(f"[DEBUG] Parameter names from tool definition: {param_names}", Fore.CYAN + Style.DIM)
                            utils.on_print(f"[DEBUG] List values: {parameters}", Fore.CYAN + Style.DIM)
                        if len(param_names) > 0 and len(parameters) > 0:
                            ...
                            if ctx.verbose:
                                utils.on_print(f"[DEBUG] Converted list to dict: {parameters}", Fore.CYAN + Style.DIM)
                        else:
                            parameters = {}
                    else:
                        if ctx.verbose:
                            utils.on_print("[DEBUG] No parameter definition found in tool, using empty dict", Fore.CYAN + Style.DIM)
                        parameters = {}
                elif not isinstance(parameters, dict):
                    # If it's neither string, list, nor dict, convert to empty dict
                    if ctx.verbose:
                        utils.on_print(f"[DEBUG] Parameters is {type(parameters)}, converting to empty dict", Fore.CYAN + Style.DIM)
                    parameters = {}

                if ctx.verbose:
                    utils.on_print(f"[DEBUG] Final parameters before tool call: {parameters} (type: {type(parameters)})", Fore.CYAN + Style.DIM)
                ...
                if ctx.verbose and accepted_params:
                    utils.on_print(f"[DEBUG] Accepted parameters from tool definition: {accepted_params}", Fore.CYAN + Style.DIM)
                ...
                if accepted_params and isinstance(parameters, dict):
                    ...
                    if ctx.verbose and original_params != parameters:
                        utils.on_print(f"[DEBUG] Filtered parameters: removed {set(original_params.keys()) - set(parameters.keys())}", Fore.CYAN + Style.DIM)
                        utils.on_print(f"[DEBUG] Parameters after filtering: {parameters}", Fore.CYAN + Style.DIM)

                # Check if the tool is a globally defined function
                if tool_name in globals():
                    if ctx.verbose:
                        utils.on_print(f"Calling tool function: {tool_name} with parameters: {parameters}", Fore.WHITE + Style.DIM)
                    try:
                        # Call the global function with extracted parameters
                        tool_response = globals()[tool_name](**parameters)
                        if ctx.verbose:
                            utils.on_print(f"Tool response: {tool_response}", Fore.WHITE + Style.DIM)
                        tool_found = True
                    except Exception as e:
                        utils.on_print(f"Error calling tool function: {tool_name} - {e}", Fore.RED + Style.NORMAL)
                else:
                    if ctx.verbose:
                        utils.on_print(f"Trying to find plugin with function '{tool_name}'...", Fore.WHITE + Style.DIM)
                    # Search for the tool function in plugins
                    for plugin in plugins.plugin_manager.plugins:
                        if hasattr(plugin, tool_name) and callable(getattr(plugin, tool_name)):
                            tool_found = True
                            if ctx.verbose:
                                utils.on_print(f"Calling tool function: {tool_name} from plugin: {plugin.__class__.__name__} with arguments {parameters}", Fore.WHITE + Style.DIM)
    ...
    if tool_found:
        ...
        utils.on_print("Tools not found", Fore.RED)
        return None

    return bot_response


def ask_openai_with_conversation(conversation, selected_model=None, temperature=0.1, prompt_template=None, stream_active=True, tools=[],  *,  ctx:Context):
    ...
    try:
        ...
        utils.on_print(f"Error during OpenAI completion: {e}", Fore.RED)
        return "", False, True
    ...
    if len(tool_calls) > 0:
        ...
        if not stream_active:
            ...
            try:
                ...
                utils.on_print(f"Error during streaming completion: {e}", Fore.RED)
                bot_response = ""
                bot_response_is_tool_calls = False
...
def generate_tool_response(user_input, tools, selected_model, temperature=0.1, prompt_template=None, num_ctx=None,  *,  ctx:Context):
    """Generate a response using Ollama that suggests function calls based on the user input."""

    rendered_tools = utils.render_tools(tools)
