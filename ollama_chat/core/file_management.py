...
def create_file(file_path, content, ctx:Context,  encoding="utf-8"):
    """
    Create a new file with the given content. The file will be tracked in the session for safe
    deletion.

    :param file_path: The full path where the file should be created. Parent dirs will be created if needed.
    ...
    try:
        ...
        if file_path not in ctx.session_created_files:
...
def delete_file(file_path,  *,  ctx:Context):
    """
    Delete a file that was created during this session. Only files created with the create_file tool
    can be deleted.
...
def read_file(file_path, ctx:Context,  encoding="utf-8"):
    """
